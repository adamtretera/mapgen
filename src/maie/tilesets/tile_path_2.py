...
# basic terrain-level adjacency (non-directional)
allowed_neighbors = {
...
class SimpleTileSet:
    ...
    def is_allowed_neighbor(self, root: int, neighbor: int, direction: NeighborIndex) -> bool:
        ...
        # if exactly one side has a path connection in this direction -> invalid
        if root_has != neigh_has:
            return False

        return True

    def draw_tile(self, tile_id: TileType, surf, x, y, scale):
